...
generalized to a multimode state. For an electromagnetic field in
some volume, we have the expansion \eqref{eqCh1_sep1}:
\begin{equation}
...
\end{equation}
The Hamiltonian function is equal to the sum of the Hamiltonian functions of all modes, as
the modes correspond to independent oscillators: 
\begin{equation}
\mathcal{H} = \frac{1}{2} 
...
\left[\hat{q}_s, \hat{q}_{s'}\right] = \left[\hat{p}_s,
  \hat{p}_{s'}\right] = 0.
\end{equation}
The Hamiltonian function then becomes the Hamiltonian operator:
\begin{equation}
...
\end{equation}
\rindex{Hamiltonian}
Using them, the Hamiltonian can be represented as
\begin{equation}
\hat{\mathcal{H}_s} = \omega_n \hbar 
\left(\hat{a}_s^{\dag} \hat{a}_s + \frac{1}{2}\right)
\nonumber
\end{equation}
The general state, when there are $n_1$ photons in the first mode, $n_2$
photons in the second, and $n_s$ photons in the $s$-th mode, etc., can be represented as
...
\nonumber
\end{equation}
where $\left\{n_s\right\}$ denotes the set of mode occupation numbers.

The action of the operators $\hat{a}_s^{\dag}$ and $\hat{a}_s$ related to the $s$-th
mode on the state vector is described by the equations 
\begin{eqnarray}
...
The state vector in general can be represented as a linear
...
When decomposing the field into plane waves, the electric field operator 
has the form
\begin{equation}
...
\label{eqCh1_multiE}
\end{equation}
This expression is written in the Heisenberg representation (the operator depends
on time). In this representation 
\[
...
\]
where $\hat{a}_k\left(0\right)$, $\hat{a}_k^{\dag}\left(0\right)$ -
operators in the Schrödinger representation. In the Schrödinger representation in
expression \eqref{eqCh1_multiE}, operators do not depend on time.
For each mode, we have 
equations \eqref{eqCh1_E_middle}, \eqref{eqCh1_E2_middle}: 
\[
\bra{n_k}\hat{a}_k\ket{n_k} = 
\bra{n_k}\hat{a}^{\dag}_k\ket{n_k} = 0
\]
From here for the full field, we obtain
\begin{equation}
...
Expression \eqref{eqCh1_multiE} can be broken into two terms:
...
\label{eqCh1_79}
\end{eqnarray} 
$\hat{\vec{E}}^{(+)}$ corresponds to the analytic signal in
the classical case. 