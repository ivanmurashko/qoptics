...
How can the phase inversion be implemented: what does the quantum logic gate look like that performs the transformation
\eqref{eqQuantCompGroverUxast}, i.e., how can $f\left(x\right)$
be ``sent'' into the phase?

\input ./part4/quantcomp/figgroverphaseinvimpl.tex

Consider the circuit shown in
\autoref{figQuantCompGroverPhaseInvImpl}. The proposed circuit
...
\ket{x}\otimes\left(\frac{\left|0\oplus 0\right> -
  \left|1\oplus 0\right>}{\sqrt{2}}\right) = 
\nonumber \\
...
Consider the circuit shown in
\autoref{figQuantCompGroverMeanInvImpl}. The element $\hat{U}_{x \ne 0}$
performs a transformation analogous to
\eqref{eqQuantCompGroverPhaseInvImpl}, but with the function
$f\left(x = 0\right) = 0$ and $f\left(x \ne 0\right) = 1$, thus
\begin{eqnarray}
...
\nonumber
\end{eqnarray}
i.e., the transformation matrix looks like:
\begin{eqnarray}
\hat{U}_{x \ne 0} = 
\begin{pmatrix}
...
Combining this result with two Hadamard transforms
\rindex{Hadamard transform}, and using
...
If we act with the operator $\hat{H}^{\otimes n}\hat{U}_{x \ne
  0}\hat{H}^{\otimes n}$, then using the result
\eqref{eqQuantCompGroverMeanInvImpl} we get:
\begin{eqnarray}
...
\nonumber
\end{eqnarray}
Thus, the circuit proposed in \autoref{figQuantCompGroverMeanInvImpl}
indeed performs inversion about the mean.
